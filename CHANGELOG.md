## 3.0.4
* iOS: 応答済みのコールを終了する際、NSUserActivityが作成されないように修正。複数のコールがある場合でも、各コールのUUIDを確認して応答済みかどうかを正しく判定する。応答済みのコールのみ、CXCallEndedReason.remoteEndedを明示的に指定することで、意図しないコールバックが発生しなくなる。
